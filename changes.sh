...
echo "🚚 Adding Carrier Registration details modal"
echo "-------------------------------------------"

if [ ! -f "src/pages/Dashboard.tsx" ]; then
  echo "❌ src/pages/Dashboard.tsx not found. Run this from the project root."
  exit 1
fi

mkdir -p backups/src/pages
cp src/pages/Dashboard.tsx "backups/src/pages/Dashboard.tsx.backup.$(date +%Y%m%d%H%M%S)"
...
echo "Click any carrier registration row to open a full details modal."
echo "The modal includes carrier info, equipment, documents, and approve/reject/pending actions."